...
class AdvancedCustomizationsSettingsController(AdvancedCustomizationsSettingsView):
    """Coordinates advanced customization state and actions for the view."""
    ...
    def _configure_signals(self):
        ...
        self.css_files.itemChanged.connect(
            lambda item: self._handle_asset_toggle(
                item, CustomizationsManager.TYPE_CSS)
        )
        self.css_files.currentItemChanged.connect(
            self._on_css_selected_for_preview)
        ...
        self.css_preview_replace_button.clicked.connect(
            self._upload_css_preview_image)
    ...
    def _refresh_current_account(self):
        ...
        if current and current.user:
            ...
            name = current.user.name if current.user.name else _(
                "Unnamed account")
    ...
    def _load_scope(self):
        ...
        if is_account_scope:
            self.inherit_checkbox.setChecked(
                CustomizationsManager.get_inherit(account_id))
        else:
    ...
    def _show_userstyle_or_default_error(self, error: Exception, default_message: str):
        if str(error) == "invalid-userstyle-target":
            self._show_feedback(
                _("This userstyle does not target WhatsApp Web."))
            return
    ...
    def _apply_selected_css_to_preview(self):
        file_name = self._selected_file_name(
            CustomizationsManager.TYPE_CSS, show_feedback=False)
        if not file_name:
            self.css_preview_placeholder_upload_button.setVisible(False)
            self._set_css_preview_placeholder(
                _("Select a CSS file to preview."))
            return
    ...
    def _show_imported_file_feedback(self, asset_type: str, source_url: str, imported_file: str):
        ...
        self._show_feedback(
            _("File imported: {}. No preview found, upload one manually.").format(
                imported_file)
        )
    ...
    def _ask_import_from_url(self, asset_type: str):
        ...
        if asset_type == CustomizationsManager.TYPE_CSS:
            url_input.setPlaceholderText(
                _("https://example.com/theme.user.css"))
        else:
    ...
    def _import_asset(self, asset_type: str):
        ...
        try:
            ...
            self._show_userstyle_or_default_error(
                error, _("Could not import file."))
            return
    ...
    def _import_asset_from_url(self, asset_type: str):
        ...
        try:
            target_path = self._import_asset_from_url_by_type(
                asset_type, url, file_name)
        except ValueError as error:
    ...
    def _create_asset(self, asset_type: str):
        ...
        try:
            ...
            self._show_userstyle_or_default_error(
                error, _("Could not create file."))
            return

        self._refresh_asset_lists()
        self._show_feedback(_("File created: {}").format(
    ...
    def _edit_selected_asset(self, asset_type: str):
        ...
        title = _("Edit CSS: {}") if asset_type == CustomizationsManager.TYPE_CSS else _(
            "Edit JavaScript: {}")
    ...
    def _handle_asset_toggle(self, item, asset_type: str):
        ...
        self._show_feedback(
            _("{}. Save and reload to apply changes.").format(
    ...
    def _save(self, show_feedback=True):
        ...
        if scope == CustomizationsManager.SCOPE_ACCOUNT:
            CustomizationsManager.set_inherit(
                account_id, self.inherit_checkbox.isChecked())
