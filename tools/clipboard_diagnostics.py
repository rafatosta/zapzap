...
def _enum_name(value: Any) -> str:
    """Return a stable readable name for Qt enums and enum-like values."""
    name = getattr(value, "name", None)
    return str(name if name is not None else value)


def collect_environment(app: QGuiApplication) -> dict[str, Any]:
    """Collect non-sensitive runtime information useful for clipboard triage."""
    return {
...
def summarize_mime_data(mime_data: Any, include_format_sizes: bool = False) -> dict[str, Any]:
    ...
    if include_format_sizes:
        for mime_type in formats:
            ...
            except Exception as exc:  # Diagnostic tool: report inability, never fail whole run.
...
def summarize_image(image: Any) -> dict[str, Any]:
    """Describe the decoded clipboard image without serializing image bytes."""
...
def render_report(environment: dict[str, Any], snapshots: list[dict[str, Any]], source: str) -> str:
    """Render diagnostics as plain text suitable for a GitHub issue attachment."""
    lines = [
